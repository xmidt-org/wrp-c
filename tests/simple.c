...
struct test_to_string {
    ...
    const ssize_t expected_string_size;
};
...
void test_to_string()
{
    ...
    for( i = 0; i < sizeof( test ) / sizeof( struct test_to_string ); i++ ) {
        char *out;
        ssize_t rv, expected_string_size;

        /* Testing to string. */
        out = wrp_struct_to_string( &test[i].in );

        if( ( NULL != out ) && ( 0 != strcmp( out, test[i].expected_string ) ) ) {
            printf( "\n\nGot: |%s| Expected: |%s|\n\n", out, test[i].expected_string );
        }

        CU_ASSERT_STRING_EQUAL( out, test[i].expected_string );
        free( out );

        rv = wrp_struct_to( &test[i].in, WRP_STRING, ( void** ) &out );

        expected_string_size = test[i].expected_string_size;

        if( 0 == expected_string_size ) {
            expected_string_size = strlen( test[i].expected_string );
        }
        ...
        if( rv != expected_string_size ) {
            printf( "\n\nGot: %zd Expected: %zd\n\n", rv, expected_string_size );
        }

        CU_ASSERT_EQUAL( rv, expected_string_size );
        free( out );
    }
}
