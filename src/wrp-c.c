...
#define CFG_FILE "/nvram/configFile.json"
...
static ssize_t __wrp_req_crud_struct_to_string( const struct wrp_req_msg *req, char ** bytes );
...
static ssize_t __wrp_pack_structure( int msg_type, char* source, char* dest,
                                     ...
                                     headers_t *headers, char **data, char *path );
...
static void decodeRequest( msgpack_object deserialized, int *msgType, char** source_ptr,
                           ...
                           int *statusValue,
                           char** payload_ptr, size_t *payload_size, bool *include_spans, char** path_ptr );
    ...
    switch( msg->msg_type ) {
        ...
        case WRP_MSG_TYPE__REQ:
            rv = __wrp_pack_structure( msg->msg_type, req->source, req->dest, req->transaction_uuid,
                                       req->include_spans, &req->spans, req->payload, req->payload_size, req->headers, bytes, NULL );
            break;
        case WRP_MSG_TYPE__EVENT:
            rv = __wrp_pack_structure( msg->msg_type, event->source, event->dest, NULL, false, NULL,
                                       event->payload, event->payload_size, event->headers, bytes, NULL );
            break;
        case WRP_MSG_TYPE__CREATE:
        case WRP_MSG_TYPE__RETRIEVE:
        case WRP_MSG_TYPE__UPDATE:
        case WRP_MSG_TYPE__DELETE:
            rv = __wrp_pack_structure( msg->msg_type, req->source, req->dest, req->transaction_uuid,
                                       req->include_spans, &req->spans, req->payload, req->payload_size, req->headers, bytes, req->path );
            break;
        default:
            break;
    }
...
static ssize_t __wrp_struct_to_string( const wrp_msg_t *msg, char **bytes )
{
    switch( msg->msg_type ) {
        ...
        case WRP_MSG_TYPE__UPDATE:
        case WRP_MSG_TYPE__DELETE:
            return __wrp_req_crud_struct_to_string( &msg->u.req, bytes );
            break;
        default:
            break;
    }
...
static ssize_t __wrp_req_crud_struct_to_string( const struct wrp_req_msg *req, char **bytes )
{
    ...
    length = snprintf( NULL, 0, req_fmt, req->transaction_uuid, req->source,
                       req->dest, headers, ( req->include_spans ? "true" : "false" ),
                       spans, req->path, req->payload_size );
                                     ...
                                     headers_t *headers, char **data, char *path )
{
    ...
    __msgpack_pack_string_nvp( &pk, &WRP_PATH, path );
            ...
            case MSGPACK_UNPACK_SUCCESS:
                ...
                if( deserialized.via.map.size != 0 ) {
                    decodeRequest( deserialized, &msgType, &source, &dest, &transaction_uuid, &headers,
                                   &statusValue, &payload, &payload_size, &include_spans , &path );
                }
                ...
                switch( msgType ) {
                    ...
                    case WRP_MSG_TYPE__DELETE:
                        ...
                        msg->u.req.headers = headers;
                        msg->u.req.include_spans = include_spans;
                        msg->u.req.spans.spans = NULL;
                        msg->u.req.spans.count = 0;
                        msg->u.req.payload = payload;
                        msg->u.req.payload_size = payload_size;
                        msg->u.req.path = path;
                        *msg_ptr = msg;
